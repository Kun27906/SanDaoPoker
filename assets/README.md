...
构建时由 CMake 自动同步到可执行文件所在目录（见 `cmake/sync_assets.cmake`）；素材变更后执行 `cmake --build build` 即完成同步。

| 目录 | 用途 |
|------|------|
| `cards/` | 扑克牌贴图：57 张（4 花色 × 13 点数 + 3 种牌背 + 大小王），PNG 200×280 |
| `cards/back/deck_pile_*` | 牌堆堆叠图（红/蓝/黑，689×292，54 层右侧露边），发牌动画用 |
| `ui/` | 界面素材：`backgrounds/` 背景图、`buttons/` 按钮四态图（UI 按钮统一使用）、`chips/` 筹码、`icons/` 图标、`table/` 桌面小贴图（倒计时贴图 + `panel_frame.png` 弹窗装饰边框）、`avatars/` 头像 |
| `ai/` | AI 胜率表 `winrate.bin`（24804 种三张牌组合；离线生成，见 `tools/gen_winrate_table`；游戏启动时加载） |
...
ui/avatars/<任意名>.png    头像图（100×100 方形；加载时按内切圆做圆形裁剪，
                           文件名排序后按序分配给各玩家）
...
| `ui/buttons/btn_normal.png` | 按钮-常态（999×60） |
| `ui/buttons/btn_hover.png` | 按钮-悬停（999×60） |
| `ui/buttons/btn_pressed.png` | 按钮-按下（999×60） |
| `ui/buttons/btn_disabled.png` | 按钮-禁用（999×60） |
| `ui/chips/chip_*.png` | 筹码 6 档区间（416×416） |
| `ui/icons/*.png` | 界面图标：menuList / musicOn / musicOff / soundSetting / soundOff / wrench / home / close（100×100）、slider（64×64） |
...
| `ui/table/panel_frame.png` | 弹窗装饰边框（九宫格切图，缩放参数见 `include/ui/PanelFrame.h`） |
...
| `bet.ogg` | 下注 | 每局开始，配合筹码数字滚动扣减 |
| `coins.wav` | 金币结算 | 每局结算 + 整场结束点击“返回大厅”，配合筹码数字跳动 |
| `error.ogg` | 操作不可用 | 发牌/组牌/比牌界面点击 wrench（禁用开发者模式提示） |
| `count_down_clock.wav` | 倒计时时钟 | 组牌倒计时进入红色区（≤15%）二倍速循环播放，交牌/超时停止 |
| `click.ogg` | 按钮点击 | 所有按钮/图标按钮通用 |
| `bgm_menu.mp3` / `bgm_game.mp3` | 背景音乐 | 菜单组 / 对局组循环（music 键开关） |

接入：`SoundManager` 单例负责加载与播放；短音效设主/副两条通道，`coins` 走副通道，可与 win/lose 叠加。
洗牌动画未采用，不含洗牌音效。
...
| 文件 | 说明 |
|------|------|
| `cards/back/deck_pile_red.png` | 红背牌堆（54 层，右侧逐层露边，689×292） |
| `cards/back/deck_pile_blue.png` | 蓝背牌堆（同上） |
| `cards/back/deck_pile_black.png` | 黑背牌堆（同上） |

生成脚本：`tools/gen_deck_pile.py`（可调层数/错开量/纸边宽度后重跑）。
用途：发牌环节展示牌堆；动画按剩余张数从右侧裁切。
