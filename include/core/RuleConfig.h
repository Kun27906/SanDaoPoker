...
// 注金：每人每局下注金 1 份
struct RoomConfig {
    const char* name;   // 房间名字
    int players;        // 人数
    int ante;           // 注金
    int rounds;         // 总轮次
};

// 房间 = 人数 ×; 注金为每人每局下注额, 总池均分三小池
...
// 0 局 -> 不罚; 1-2 局 -> 罚 1 局底注; 3-4 局 -> 2 局; 5-8 局 -> 3 局; 9 局及以上 -> 4 局
// playedRounds 只统计【已结算完成】的局 —— 局内提前退出时当前这局不计入。
